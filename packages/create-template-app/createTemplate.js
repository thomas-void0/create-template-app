...
function init() {
  ...
  createApp({
    ...
    usePnp: options.usePnp,
    useYarn: isUsingYarn(),
  })
}

function createApp({ name, verbose, template, useYarn, usePnp }) {
...
function run({
  ...
  getTemplateInstallPackage(template, originalDirectory).then(templateToInstall => {
    getPackageInfo(templateToInstall)
      ...
      .then(({ templateInfo }) => {
        downloadTemplate(root, appName, verbose, originalDirectory, templateInfo.name)
      })
      .catch(reason => {
        console.log();
...
function downloadTemplate(
  ...
  const templateDir = path.join(templatePath);
...
function executeNodeScript2({ cwd, args }, data, source) {
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
